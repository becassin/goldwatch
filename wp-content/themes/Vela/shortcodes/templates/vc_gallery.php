 ...
 */
$title = $source = $type = $columns = $onclick = $custom_links = $custom_links_target = $img_size = $external_img_size = $images = $custom_srcs = $el_class = $interval = $css = '';
$large_img_src = '';
...
if ( 'nivo' === $type ) {
	...
	$col_name = 'col-sm-'.  absint( floor(12/ intval( $columns ) ) );
	$el_start = '<li class="isotope-item item '.$col_name.'">';
	$el_end = '</li>';
	...
	switch ( $source ) {
		case 'media_library':
			if ( $image > 0 ) {
				...
				$p_img_large = wp_get_attachment_image_src( $image, 'full' );
				$large_img_src = $p_img_large[0];
			} else {
